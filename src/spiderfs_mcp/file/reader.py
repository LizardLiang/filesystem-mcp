...
class FileReader:
    ...
    def read_file(self, path: str, encoding: Optional[str] = None) -> str:
    ...
    def read_line_range(
        self, file_path: str, line_range: LineRange, encoding: Optional[str] = None
    ) -> FileReadResult:
        """
        ...
        try:
            ...
            if not path_obj.is_file():
                return FileReadResult(
                    content="", line_range=line_range, error=f"Not a file: {file_path}"
                )

            # Validate line range
            if line_range.start < 1:
                return FileReadResult(
                    content="", line_range=line_range, error="Line numbers must be >= 1"
                )
            ...
            return FileReadResult(
                content="", line_range=line_range, error=f"Error reading file: {str(e)}"
            )

    def read_context_around_line(
        self,
        file_path: str,
        line_number: int,
        context_lines: int = 3,
        ...
        result.metadata.update(
            {"target_line": line_number, "context_lines": context_lines}
        )

        return result

    def detect_encoding(self, file_path: str, sample_size: int = 1024) -> str | None:
        try:
            ...
            # If not UTF-8, try Latin-1 (always works)
            decoded = raw_data.decode("latin1")

            # Check if it has typical Latin-1 chars (optional)
            latin1_chars = set("éàèùìòçñäöüß")
            if any(char in decoded for char in latin1_chars):
                return "latin1"

            # Fall back to chardet if needed
            result = from_bytes(raw_data).best()
            return result.encoding.replace("_", "-") if result else None
        except Exception as e:
