...
class ReadLineRangeRequest(BaseModel):
    ...
    encoding: Optional[str] = Field(
        None, description="File encoding (e.g., 'utf-8', 'latin-1')"
    )
...
class ReadLineContextRequest(BaseModel):
    ...
    context_lines: int = Field(
        3, ge=0, description="Number of context lines before and after"
    )
    encoding: Optional[str] = Field(
        None, description="File encoding (e.g., 'utf-8', 'latin-1')"
    )
...
async def stream_file_by_lines(
    file_path: str, chunk_size: int = 1000, encoding: Optional[str] = None
):
    """
    ...
    async def content_generator():
        for content_chunk, metadata in streamer.stream_file_by_lines(
            file_path, encoding=encoding
        ):
            # Create a JSON response for each chunk
            chunk_response = StreamChunkResponse(chunk=content_chunk, metadata=metadata)
            # Convert to JSON and add a newline as a delimiter
            yield json.dumps(chunk_response.model_dump()) + "\n"

    return StreamingResponse(content_generator(), media_type="application/x-ndjson")


@router.get("/stream/bytes/{file_path:path}")
async def stream_file_by_bytes(
    file_path: str, chunk_size: int = 8192, binary_mode: bool = True
):
    """
    ...
    async def content_generator():
        for chunk, _ in streamer.stream_file_by_bytes(
            file_path, chunk_size, binary_mode=binary_mode
        ):
            yield chunk
...
class ApplyLineEditsRequest(BaseModel):
    ...
    create_backup: bool = Field(
        True, description="Whether to create a backup before writing"
    )
    encoding: Optional[str] = Field(
        None, description="File encoding (e.g., 'utf-8', 'latin-1')"
    )
...
class ReplaceStringRequest(BaseModel):
    ...
    max_replacements: int = Field(
        0, ge=0, description="Maximum number of replacements (0 = unlimited)"
    )
    create_backup: bool = Field(
        True, description="Whether to create a backup before writing"
    )
